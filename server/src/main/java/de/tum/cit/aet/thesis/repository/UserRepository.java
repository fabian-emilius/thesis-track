...
public interface UserRepository extends JpaRepository<User, UUID> {
    ...
    @Query("SELECT u FROM User u WHERE u.joinedAt < :cutoffDate AND u.updatedAt < :cutoffDate " +
           "AND NOT EXISTS (SELECT 1 FROM Application a WHERE a.user = u AND a.createdAt >= :cutoffDate) " +
           "AND NOT EXISTS (SELECT 1 FROM ThesisRole tr WHERE tr.user = u AND tr.assignedAt >= :cutoffDate)")
    List<User> findUsersForDeletion(@Param("cutoffDate") Instant cutoffDate);
}
