...
export function GroupProvider({ children }: GroupProviderProps) {
  ...
  const checkRole = useCallback((groupId: string, role: GroupRole): boolean => {
    return userGroups.some(membership => 
      membership.groupId === groupId && membership.role === role
    );
  }, [userGroups]);

  const isGroupAdmin = useCallback((groupId: string): boolean => 
    checkRole(groupId, 'GROUP_ADMIN'), [checkRole]);

  const isSupervisor = useCallback((groupId: string): boolean => 
    checkRole(groupId, 'SUPERVISOR'), [checkRole]);

  const isAdvisor = useCallback((groupId: string): boolean => 
    checkRole(groupId, 'ADVISOR'), [checkRole]);
  ...
  const handleGroupUpdate = useCallback(async (groupId: string, data: Partial<Group>): Promise<void> => {
    try {
